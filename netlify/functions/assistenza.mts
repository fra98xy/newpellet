...
export default async (req: Request) => {
  ...
  try {
    ...
    const backgroundUrl = new URL("/.netlify/functions/send-assistenza-email-background", req.url).toString();
    fetch(backgroundUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, phone, problem })
    }).catch(e => console.error("Failed to invoke assistenza background function", e));
